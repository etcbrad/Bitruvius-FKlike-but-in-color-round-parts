  ...
  for (let x = startX; x <= endX; x += minorGridSize) {
    ...
    lines.push(
      <line
        ...
        stroke={isMajor ? 'rgba(141, 212, 153, 0.3)' : 'rgba(141, 212, 153, 0.15)'}
  ...
  for (let y = startY; y <= endY; y += minorGridSize) {
    ...
    lines.push(
      <line
        ...
        y2={y}
        stroke={isMajor ? 'rgba(106, 92, 145, 0.3)' : 'rgba(106, 92, 145, 0.15)'}
        strokeWidth={isMajor ? 1 : 0.5}
      />
    );
  }
...
export const SystemGuides: React.FC<SystemGuidesProps> = ({ floorY, isAirMode }) => {
  const guideColor = 'rgba(106, 92, 145, 0.25)';
  ...
  return (
    <g className="pointer-events-none">
      ...
      {/* Floor guide line and ground strip, only visible if not in air mode */}
      {!isAirMode && (
